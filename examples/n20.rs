...
fn main() -> anyhow::Result<()> {
    ...
    let m1_forward = LedcDriver::new(peripherals.ledc.channel0, driver1, peripherals.pins.gpio4)?;

    let m1_reverse = LedcDriver::new(peripherals.ledc.channel1, driver2, peripherals.pins.gpio5)?;

    let m2_forward = LedcDriver::new(peripherals.ledc.channel2, driver3, peripherals.pins.gpio6)?;
    let m2_reverse = LedcDriver::new(peripherals.ledc.channel3, driver4, peripherals.pins.gpio7)?;
    // enabled by default
    let m1_enable = peripherals.pins.gpio35;
    let m2_enable = peripherals.pins.gpio36;
    ...
    // Encoder
    let mut m1_pin_a = peripherals.pins.gpio21;
    let mut m1_pin_b = peripherals.pins.gpio20;
    let mut m2_pin_a = peripherals.pins.gpio19;
    let mut m2_pin_b = peripherals.pins.gpio42;
